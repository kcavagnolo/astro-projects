pro make_1T_script,nregions,nH_gal,kT_guess,ab_guess,redshift,lo_energy,hi_energy,bin
;-----------------------------------------------------------------------
;
; Name: MAKE_1T_SCRIPT
;
...
for i=1,nreg-1 do begin
   thaw_ab=thaw_ab+','+strtrim(string(4+i*10),2)
   freeze_wabsnH=freeze_wabsnH+','+strtrim(string(1+i*10),2)
endfor
...
for i=1,nreg do begin
   printf,unit,'error $Conf_Level_Par '+strtrim(string(2+(i-1)*10),2)+' maximum 10.0'
   printf,unit,'error $Conf_Level_Par '+strtrim(string(4+(i-1)*10),2)+' maximum 10.0'
   printf,unit,'error $Conf_Level_Par '+strtrim(string(7+(i-1)*10),2)+' maximum 10.0'
endfor
for i=1,nreg do begin
   printf,unit,'tclout error '+strtrim(string(2+(i-1)*10),2)
   printf,unit,'scan $xspec_tclout "%f %f" p'+strtrim(string(2+(i-1)*10),2)+'_err_low p'+strtrim(string(2+(i-1)*10),2)+'_err_high'
   printf,unit,'tclout error '+strtrim(string(4+(i-1)*10),2)
   printf,unit,'scan $xspec_tclout "%f %f" p'+strtrim(string(4+(i-1)*10),2)+'_err_low p'+strtrim(string(4+(i-1)*10),2)+'_err_high'
...
for i=1,nreg do begin
   printf,unit,'tclout param '+strtrim(string(2+(i-1)*10),2)
   printf,unit,'scan $xspec_tclout "%f" kT'+strtrim(string(i),2)+'_result'
   printf,unit,'tclout param '+strtrim(string(4+(i-1)*10),2)
...
printf,unit,'}'
for i=1,nreg do begin
   printf,unit,'puts $fileid "1_sigma_conf_ranges for kT (region '+strtrim(string(i),2)+'):($p'+strtrim(string(2+(i-1)*10),2)+'_err_low - $p'+strtrim(string(2+(i-1)*10),2)+'_err_high)"'
   printf,unit,'puts $fileid "1_sigma_conf_ranges for Abundanc (region '+strtrim(string(i),2)+'):($p'+strtrim(string(4+(i-1)*10),2)+'_err_low - $p'+strtrim(string(4+(i-1)*10),2)+'_err_high)"'
   printf,unit,'puts $fileid "1_sigma_conf_ranges for Norm (region '+strtrim(string(i),2)+'):($p'+strtrim(string(7+(i-1)*10),2)+'_err_low - $p'+strtrim(string(7+(i-1)*10),2)+'_err_high)"'
endfor
...
for i=1,nreg do begin
   ...
   printf,unit,'  puts $fileid "KT'+strtrim(string(i),2)+'ERRU=$p'+strtrim(string(2+(i-1)*10),2)+'_err_high / \[keV\] Energy 1sigma upper limit"'
   printf,unit,'  puts $fileid "KT'+strtrim(string(i),2)+'ERRL=$p'+strtrim(string(2+(i-1)*10),2)+'_err_low / \[keV\] Energy 1sigma lower limit"'
   printf,unit,'  puts $fileid "AB'+strtrim(string(i),2)+'=$ab'+strtrim(string(i),2)+'_result / Derived abundance"'
   printf,unit,'  puts $fileid "AB'+strtrim(string(i),2)+'ERRU=$p'+strtrim(string(4+(i-1)*10),2)+'_err_high / Abundance 1sigma upper limit"'
   printf,unit,'  puts $fileid "AB'+strtrim(string(i),2)+'ERRL=$p'+strtrim(string(4+(i-1)*10),2)+'_err_low / Abundance 1sigma lower limit"'   
   printf,unit,'  puts $fileid "NORM'+strtrim(string(i),2)+'=$norm'+strtrim(string(i),2)+'_result / Derived normalization"'
   printf,unit,'  puts $fileid "N'+strtrim(string(i),2)+'ERRU=$p'+strtrim(string(7+(i-1)*10),2)+'_err_high / Normalization 1sigma upper limit"'
   printf,unit,'  puts $fileid "N'+strtrim(string(i),2)+'ERRL=$p'+strtrim(string(7+(i-1)*10),2)+'_err_low / Normalization 1sigma lower limit"'
endfor
