pro make_xray_flux_script,nregions,ncool,nH_gal,kT_guess,ab_guess, $
          redshift,lo_energy,hi_energy,bin,inner_a,inner_b, $
          inner_pa
;-----------------------------------------------------------------------
;
; Name: MAKE_XRAY_FLUX_SCRIPT
;
...
;
get_lun,unit
openw,unit,'projct_wabs_mekal_flux.xcm'
printf,unit,'# XSPEC script for finding the PROJCT*MEKAL*WABS flux inside'
printf,unit,'# the cooling radius.'
...
;
printf,unit,' '
printf,unit,'set model_name "projct_wabs_mekal_flux"'
...
printf,unit,'set fitsfl "source_mekal_flux.spectra"'
...
printf,unit,'file copy -force z_temp_xspec.log xspec_mekal_flux.log'
...
printf,unit,'set fileid [open xspec_mekal_flux.log a]'
printf,unit,'for {set ii 0} {$ii <=[expr $BandNum*2-1]} {incr ii} {'
printf,unit,'  if {$ii < $BandNum} {'
