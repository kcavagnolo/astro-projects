        ...
        char *argv[];
{
        ...
        float   aexpn,zobs,*dx,*map,*vector();
	FILE    *inp,*out;
	...
	/* Input datafile */
        if (argc<5) {printf("Input <in filename> <out filename> <zobs> <frequency [GHz]>\n");exit(0);}
	sprintf(infile,"%s",argv[1]);
	sprintf(outfile,"%s",argv[2]);
	sscanf(argv[3],"%f",&ZOBS);  /* Observing redshift */
	sscanf(argv[4],"%f",&FREQ);  /* Frequency [GHz] */

	/* Define vectors */
	nx=(int *)calloc(3,sizeof(int));
	dx=(float *)calloc(3,sizeof(float));
	
	/* Read in C-Binary datafile */
        inp=fopen(infile,"rb");
...
void writeimage( char *outfile, float *map, int nsize )
{
    ...
    if (!strncmp(&(outfile[0]),"compy",5)) flag=COMPY; 
    else if (!strncmp(&(outfile[0]),"ksz",3)) flag=KSZ; 

    /* convert compton-y and kSZ-b parameters to temperature in Kelvin */
    /* at a given frequency [GHz] */
    xfreq = FREQ * 1.0e9 * hP / ( kB * T_CMB );
    for (ii=0;ii<nsize;ii++) for (jj=0;jj<nsize;jj++) {
      /* Read in y-axis first */
      kk=ii*nsize+jj;
      if ( flag == COMPY )  map[kk] = f_tsz(xfreq) * map[kk] * T_CMB;
      else if ( flag == KSZ ) map[kk] = -1.0 * map[kk] * T_CMB;
    }
    ...
    if ( flag == COMPY ) printf("Thermal SZE data in Kelvin at %5.2f GHz \n",FREQ);
    if ( flag == KSZ ) printf("Kinematic SZE data in Kelvin at %5.2f GHz \n",FREQ);
    printf("  dA(zobs=%5.3f) = %7.3f [Mpc] \n",ZOBS,dAobs);
    printf("  FOV = %f [arcmin] \n",fov*60.0);
    printf("  Pixel size = %f [arcsec] \n",degpix*3600.0);
    if ( flag == COMPY ) printf("  f_tsz(FREQ=%4.1f GHz) = %7.5f \n",FREQ,f_tsz(xfreq)); 
    ...
    if ( flag == COMPY ) {
      if ( fits_update_key(fptr, TSTRING, "telescop", "Simulated tSZ map",
			 " ", &status) )
	printerror( status ); 
    } else if ( flag == KSZ ) {
      if ( fits_update_key(fptr, TSTRING, "telescop", "Simulated kSZ map",
			 " ", &status) )
	printerror( status ); 
    }
