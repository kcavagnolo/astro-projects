...
def readSpectra(prefix, suffix, minspec, numspecs, minradius):
    ...
    for filename in filenames:
        ...
        if specinfo.backfile.lower() != 'none':
            ...
            specinfo.backspec = Bundle(spec=specinfo.spec*0., errs=specinfo.errs*1e-10,
...
def deprojectSpectra(spectra):
    ...
    for shellnum in xrange(len(spectra)-1, -1, -1):
        ...
        totalspec = spec.spec
        deprojspec = totalspec-(spec.backspec.spec*(spec.backscal/spec.backspec.backscal))-projspec
...
def makeMonteCarloRealisations(spectrain):
    ...
    for spec in spectrain:
        ...
        speccopy.backspec.spec = N.random.normal(spec.backspec.spec,
                                                 N.where(spec.backspec.errs == 0.,
                                                         1e-99, spec.backspec.errs))
...
def writeSpectrum(outfile, spectrum, errors, grouping,
    ...
    channelcol = pyfits.Column(name='CHANNEL', format='J', array=N.arange(1, numchans+1))
    ...
    qualitycol = pyfits.Column(name='QUALITY', format='I', array=N.zeros(numchans))
    ...
    spechdu = pyfits.new_table([channelcol, ratecol, rateerrcol, qualitycol, groupcol])
    spechdr = spechdu.header

    for i in (('EXTNAME', 'SPECTRUM'),
...
def doDeprojection(spectra, iterations, minspec, outprefix, outsuffix):
    ...
    for spec, err, inspecfile in itertools.izip(specs, errors, spectra):
        writeSpectrum('%s%i%s' % (outprefix, num, outsuffix), spec, err, spectra[0].grouping,
