...
files[1] = '../me_fits/dat/final_r2500-50_fefree_2-7.dat'
files[2] = '../me_fits/dat/final_r2500_fefree_7-7.dat'
files[3] = '../me_fits/dat/final_r2500_fefree_2-7.dat'
...
FOR j = 0,n_elements(pstitle)-1 DO BEGIN
    ...
    FOR ss = 0,round(max(x)/bsize) DO BEGIN
        ...
        ENDIF
        mm++
        IF binmax LT 1. THEN binsize=bsize ELSE binsize=0.5
        binmin = binmax
    ...
    IF (m EQ 0) THEN BEGIN
        set_plot,'PS'
        device, filename = "ftzbin.ps"
        !fancy = 4
        !linetype = 0
        !p.font = 0
        multiplot,[2,2],/rowmajor
    ENDIF
    ...
    IF (m NE 0) THEN void,uppery
    IF (m NE 0) THEN void,upperx
