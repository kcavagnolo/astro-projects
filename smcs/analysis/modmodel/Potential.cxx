...
Hydrostatic::MIAtmosphere::MIAtmosphere (const std::vector<Real>& params,
  ...
  if (perShellParams % 2) {
    throw RedAlert ("Odd number of per shell parameters - check model.dat.");
  }
  size_t nShells = numShells ();
  if (perShellParams < 2 * nShells) {
    throw RedAlert ("Insufficient model shells for data.  Edit model.dat.");
  }
  rho.resize (nShells);
...
void Hydrostatic::NFWAtmosphere::pdump () const {
  std::cerr << "nfwa: " << NFWScale << ", nfwnorm: " << NFWNorm << "\n";
}
