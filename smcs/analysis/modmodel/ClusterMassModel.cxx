...
void ClusterMassModel::verifyData () const {
  // Checks for:
  // data present
  ...
  for (size_t iSpec = 1; iSpec <= nTotalSpec; ++iSpec) {
	...
	sectorTest [groupNum] = testSector (spec->xflt (), iSpec);
      } else {
	Sector currentSector (testSector (spec->xflt (), iSpec));
...
void ClusterMassModel::createSectorDescription (size_t observation, 
  ...
  const std::map<size_t, Real>& filterKeys = spectrum->xflt ();  
  ...
  if (N == 1) {
    ...
    if (b != currShell.rout) {
      string diag(" clmass requires circular annuli");
      throw IncompatibleData (diag);
    }
...
void ClusterMassModel::makeBetaEM (const std::vector<Real>& params, 
  ...
  if (beta <= (1.0 / 6.0)) {
    throw YellowAlert ("Must have beta > 1/6 in clmass.");
  }
...
void ClusterMassModel::initialize (const std::vector<Real>& params, 
  ...
  for (size_t kRadial = 0; kRadial < m_numberOfShells; ++kRadial) {
	...
	throw RedAlert ("Coding error in clmass initialize.");
      }
