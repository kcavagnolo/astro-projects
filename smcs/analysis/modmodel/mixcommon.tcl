...
# Returns:
# number of fixed pars
# maximum number of shells
# number of active shells
#
proc frontend {fxName mxName shName mxmName} {
    upvar $fxName fixedpars $mxName maxshells $shName shells $mxmName mixname
    ...
    # Maximum number of shells
    set maxshells [${mixname}MaxShells $mixpars]
...
############################################################
#
# Parameter linking and freezing required by clmass and monomass.
# Arguments give settings for the beta model (defaults should
# match model.dat).
#
proc massmod_start {{rinner 0} {a 0} {beta 0.7} {sw 0}} {
    # Sanity checks, etc
    if {[frontend fixedpars maxshells shells mixname]} {
	return
    }
...
proc denPars {rbName} {
    ...
    # One less than the number of the first density parameter
...
proc phyScale {dang radialScale} {
    set arcsec [expr [mixmod_pi] / (180.0 * 3600.0)]
...
proc fixedDen {rhofix mass} {
    return [expr pow (0.75 * $mass / ([mixmod_pi] * $rhofix), 1.0 / 3.0)]
}
#
############################################################
#
