...
c
	...
	rewind 22
      ...
      CALL LOOP1(NIND,NATT,1,X,XMU,XVAR,NCOV,IER,XML)
      IF (IER.NE.0) GO TO 605
C     This subroutine calculates the log likelihood for NG=1 and stores
C     it in XML(1)
      WRITE (22,1116)
1116  FORMAT (//2X,'Estimated mean (as a row vector) for 1-group fit:')
      DO 20 K=1,1
20    WRITE (22,11103) (XMU(K,J),J=1,NATT)
11103   FORMAT (2X,10F16.6)
	WRITE(22,*)

      DO 11130 J=1,NATT
11130   WRITE (22,104) (XVAR(1,I,J),I=1,J)
...
106   FORMAT (//2X,'Estimated mean (as a row vector) for each group')
      DO 2000 K=1,NG
2000    WRITE (22,103) (XMU(K,J),J=1,NATT)
...
105   FORMAT (//2X,'Estimated covariance matrix for group ',I2)
      DO 30 J=1,NATT
30    WRITE (22,104) (XVAR(K,I,J),I=1,J)
104   FORMAT (5X,10F16.6)
      GO TO 609
600   WRITE (22,107)
107   FORMAT (//2X,'Estimated common covariance matrix ')
      DO 40 J=1,NATT
40    WRITE (22,104) (XVAR(1,I,J),I=1,J)
      GO TO 609
605   WRITE (22,101) IER
...
30    XBAR(K,J)=XBAR(K,J)/N(K)
      T(K)=N(K)
      T(K)=T(K)/FLOAT(NIND)
35    WRITE (22,103) (XBAR(K,J),J=1,NATT)
...
105   FORMAT (//2X,'Estimated covariance matrix for group ',I4)
      DO 60 J=1,NATT
60    WRITE (22,103) (XVAR(K,I,J),I=1,J)
C     Test if a common covariance matrix is specified (NCOV = 1)
      IF (NCOV.EQ.1) GO TO 600
...
115   FORMAT (//2X,'Estimated common covariance matrix ')
      DO 878 J=1,NATT
878   WRITE (22,103) (XVAR(1,I,J),I=1,J)
...
80    WRITE (22,103) (XBAR(K,J),J=1,NATT)
C     Test if a common covariance matrix is specified (NCOV = 1)
...
90    WRITE (22,103) (XVAR(K,I,J),I=1,J)
      CALL GDET(NATT,NG,XVAR,V,DV,IER)
      IF (IER.NE.0) RETURN
      GO TO 812
700   CONTINUE
...
95    WRITE (22,103) (XVAR(1,I,J),I=1,J)
      CALL GDET(NATT,1,XVAR,V,DV,IER)
      IF (IER.NE.0) RETURN
      DO 96 K=2,NG
      DV(K)=DV(1)
...
40    CONTINUE
      ...
      SUBROUTINE LOOP1(NIND,NATT,NG,X,XMU,XVAR,NCOV,IER,XML)
