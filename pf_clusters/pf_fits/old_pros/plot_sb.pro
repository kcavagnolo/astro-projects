...
; plot the profile
set_plot, 'PS'
device, filename = strcompress(obs,/remove_all)+'_sbr_'+pix+'pix.ps'
!fancy = 4
!psym = 10
!linetype = 0
plot, rmid, sbr, /XLOG, /YLOG, $
      xtitle = "Radius (arcseconds)", $
      ytitle = "Surface Brightness (cts/arcsec!E2!N)", $
      xrange = [xmin1,xmax1], $
      yrange = [ymin,ymax], $
      xstyle = 9, $
      xcharsize = 0.8, $
...
SPAWN, 'ls *sbr*.ps > list'
SPAWN, 'cat list | perl pscat temp.ps'
SPAWN, 'psnup -4 temp.ps '+pix+'pix_sbr_profs.ps'
