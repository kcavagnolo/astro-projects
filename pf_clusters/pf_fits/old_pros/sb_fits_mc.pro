...
pro sb_fits_mc, dat1, dat2, nmc
; sb_fits_mc, '1tpr_nhfrozen_fefree.dat', 'reference_all.list', 1000

; restore the fit template and read some variables
restore,"xspecfittemplate.sav"
fitfile = read_ascii(dat1, template = xspecfittemplate)
...
; define the filename for the radial profile
    filename = '../radprofs/5pixs/'+clustername+'_'+obs+'_'+chip+'_radprof_5pix.fits'
...
; plot entropy profile and set limits on the fit
    ...
    set_xy
    plot,rmean,s,$
      xtitle="R!dmean!n (Mpc)",$
      ytitle="S (keV cm!u2!n)",$
      psym=0 & !linetype=0
    oploterr,rmean,s,s_err
    read,"Enter R limits, Rmin, and Rmax, in units of Mpc:", Rminfit, Rmaxfit
    wfit = where( (rmean ge Rminfit) * (rmean le Rmaxfit))
    print,n_elements(wfit), " points will be fit."

; fit profile
    a=[10,.0035,1.5]            ; first guess for S0, norm, slope
    result = lmfit( rmean(wfit), s(wfit), a, measure_errors=s_err(wfit), chisq=chisq_entropy,sigma=sigma_entropy, function_name="fit_form")
    prob_est = 1.0-IGAMMA((0.5*n_elements(rmean(wfit))-3), 0.5*chisq_entropy)
    print, 'S0 (keV cm^2)      r_scale (Mpc h70)    Power-law'
    print,a,'Errors:', sigma_entropy, 'Chisq: ', chisq_entropy, 'Prob: ', prob_est

; run again, but fix a=0.0
    a=[0.0, a(1), a(2)]
    fita=[0,1,1]          ; fit 2nd 2 parameters, hold first one fixed
    result_s0 = lmfit( rmean(wfit), s(wfit), a, measure_errors=s_err(wfit), fita=fita, chisq=chisq_entropy,sigma=sigma_entropy, function_name="fit_form")
    prob_est = 1.0-IGAMMA((0.5*n_elements(rmean(wfit))-2), 0.5*chisq_entropy)
    print, 'S0 (keV mc^2)      r_scale (Mpc h70)    Power-law'
    print,a,'Errors:', sigma_entropy, 'Chisq: ', chisq_entropy, 'Prob: ', prob_est

; run BOTH again, but use the flat central temperature value
    print,'FOR FLAT CENTRAL TEMPERATURES INSTEAD OF EXTRAPOLATION:'
    result_flat = lmfit( rmean(wfit), s_flat(wfit), a, measure_errors=s_err(wfit), chisq=chisq_entropy,sigma=sigma_entropy, function_name="fit_form")
    prob_est = 1.0-IGAMMA((0.5*n_elements(rmean(wfit))-3), 0.5*chisq_entropy)
    print, 'S0 (keV mc^2)      r_scale (Mpc h70)    Power-law'
    print,a,'Errors:', sigma_entropy, 'Chisq: ', chisq_entropy, 'Prob: ', prob_est
...
; run again, but fix a=0.0
    a=[0.0, a(1), a(2)]
    fita=[0,1,1]          ; fit 2nd 2 parameters, hold first one fixed
    result_s0_flat = lmfit( rmean(wfit), s_flat(wfit), a, measure_errors=s_err(wfit), fita=fita, $
                            chisq=chisq_entropy,sigma=sigma_entropy, function_name="fit_form")
    prob_est = 1.0-IGAMMA((0.5*n_elements(rmean(wfit))-2), 0.5*chisq_entropy)
    print, 'S0 (keV mc^2)      r_scale (Mpc h70)    Power-law'
    print,a,'Errors:', sigma_entropy, 'Chisq: ', chisq_entropy, 'Prob: ', prob_est
...
; write a FITS file with rmean, entropy
    ...
    hdr = ['RMEAN = mean radius in Mpc', 'S = entropy in keV cm^2', 'SERR = entropy error', 'junk = junk']
    mwrfits, tarr, obs+'_sfit.fits', hdr, /create

    !xtitle = 'R (Kpc)'
    !ytitle = 'S (keV cm!u2!n)'
    !mtitle = name
    !fancy=4                  ; for nicer fonts and larger axis labels
    set_xy,0.0001,1.000,1,1000
    !linetype=0 & !psym=1
    plot_oo,(rmean*1000.),s, $
      xrange = [1,max(rmean)+100], $
      yrange = [1,max(s)+100]
    oploterr,(rmean*1000.),s,s_err
    !psym=0
    !linetype=1
    oplot,(rmean*1000.)(wfit),result
    !linetype=2
    oplot,(rmean*1000.)(wfit),result_s0 ; fit where S(0) fixed to 0.0
    !psym=4
    oplot,(rmean*1000.),s_flat
    oploterr,(rmean*1000.),s_flat,s_err
    !psym=0 & !linetype=0
    oplot,(rmean*1000.)(wfit),result_flat
    !linetype=4
    oplot,(rmean*1000.)(wfit),result_s0_flat

    yesno=""
    read,"Want a hardcopy (postscript) plot of this? (yes=y)", yesno
    if (yesno eq "y") then begin
        plotfile = obs+'_ent.ps'
        set_plot,'PS'
