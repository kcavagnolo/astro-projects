...
\newcommand{\mykeywords}{galaxies: active -- galaxies: clusters: general -- X-rays:
...
\usepackage[pagebackref,
  ...
  pdfsubject={Astrophysical Journal Letters},
...
\begin{abstract}
  ...
  that in relativistic electrons is $\ga 100$. We identify a sub-set
  of gEs with unusually large radio luminosities for their jet
  powers. For this gE sub-set, the radio sources extend well beyond
  the densest parts of their X-ray halos, and we suggest these systems
  are unusually luminous because they did not entrain appreciable
  amounts of gas from their environments.
\end{abstract} 
...
which can exceed the energy in cavities. The cavity power estimates
were summed for all the cavities in each system to obtain an estimate
of the average AGN outburst power. Because the uncertainties are large
(chiefly due to the uncertain volume estimates), errors are propagated
in log space, assuming that the uncertainties in ambient pressure,
...
\plow\ and \phigh, respectively. The redshift dependent $\alpha$
correction in $\nu_0 L_{\nu_0}$ is small for our sample since the
objects are nearby. We have assumed the radio spectra behave as
$S_{\nu} \propto \nu^{-\alpha}$ with a spectral index of $\alpha =
0.8$, typical for extragalactic radio galaxies
...
(PC). While the lobes do not appear to be confined by the X-ray
...
mean value observed in the outskirts of clusters and groups (see the
...
directly calculated from the data, but to maintain consistency with
the rest of the study, $t_{\mathrm{buoy}}$ was estimated by scaling
the gas sound speed by 0.65 which is the mean value of the ratio
...
they would have lower radio power because a larger fraction of their
energy flux would be carried in non-radiating particles. Another
...
in clusters \citep{mcnamrev}, but the incidence and variety of AGN
