...
@directories = qq{ /home/cavagnolo/research };
$drive = '/mnt/RASPUTIN';
$destination = "${drive}/";
...
# Run Rsync
$logfile = "/home/cavagnolo/admin/rasputin_rsync.log";
open(A,">$logfile");
print A "Started back-up at local time: ",scalar(localtime),"\n";
close A;
foreach $d (@directories) {
    print STDERR "Backing up $d to $destination";
    $pid = system("rsync -aEvu $d $destination >> /home/cavagnolo/admin/rasputin_rsync.log");
    waitpid($pid,0);
    print STDERR ".\n";
}
open(A,">>$logfile");
