...
class mpfit:
   ...
   def __init__( self, fcn, xall = None, functkw = {}, parinfo = None,
                ftol = 1.e-10, xtol = 1.e-10, gtol = 1.e-10, dblprec = True,
 ...
 Outputs:
   ...
   .perror
      ...
      *If* you can assume that the true reduced chi-squared value is unity --
      ...
      self.nfev = 0
      self.damp = damp
      self.machar = machar( dblprec = dblprec )
...
#                  print self.covar.shape, ifree, indices, cv
                  self.covar[ : , ifree[ i, 0 ] ] = aput( 
                      self.covar[ : , ifree[ i, 0 ] ], ifree, cv[ : , i ] )

               ## Compute errors in parameters
               catch_msg = 'computing parameter errors'
               self.perror = zeros( shape = ( nn ), dtype = self.dtype )
...
class machar:
   ...
   def __init__( self, dblprec = True ):
