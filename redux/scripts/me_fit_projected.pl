...
$quiet     = "yes";
$rootdir   = "reprocessed/";
$specdir   = "$ENV{HOME}/research/iras09/data/";
...
# details of fitting
#@runname   = qw(r200 r500 r1000 r2500 r5000 r7500 r200-core r500-core r1000-core r2500-core r5000-core r7500-core);
#@runname   = qw(r200-core r1000-core r2500-core r5000-core r7500-core);
#@runname   = qw(r200 r500 r1000 r2500 r5000 r7500);
#@runname   = qw(r500-core);
@runname = "rcool";
...
foreach $eming (@eming) {
  foreach $runname (@runname) {
    ...
    foreach $key (sort keys %refdata) {
      ...
      $fe      = $data[9];
      $emin    = $eming;
      $emin    = sprintf("%.2f",2.0/(1+$z)) if ($eming eq "2.0");
      $crmin   = $emin;
      $datadir = $data[15];
      ...
      $databgd   = "${obsid}_local.pi" if ($uselocal eq "yes");
	...
	next;
      ...
      format_output($data,@clpar) if ($dofit eq "yes");
...
#  if ($freeze_z eq "no") {
      printf FITFILE "%-25s %6s %6s %6s %5s %5s %5s %5s %5s %5s %5s %5s %5s %10s %10s %10s %5s %5s %5s %10s %10s %10s %7s %7s %7s %6s %6s %6s %6s\n",
...
sub make_xcmfile {
  ...
  # Raymond-Smith #######################################################
  if ($model eq "ray") {
    ($nhpar,$txpar,$fepar,$zpar,$normpar) = (1,2,3,4,5);
    $modstring = "${nhmod}(ray)";
  ...
  # MEKAL+MEKAL model###################################################
    ...
    ($nhpar,$txpar,$fepar,$zpar,$normpar,$txpar2,$normpar2) = (1,2,4,5,7,8,13);
...
sub run_xspec {
 ...
 MAIN: while (<XSPEC_OUTPUT>) {
	...
	if ($line =~ m/^\s+\d+\s+/) {
      ...
      unlink "${obsid}_${runname}_spectrum.ps";
      myprint
	  "setplot energy\n",
	  "iplot ldata resid\n",
	  "label top $name $obsid\n",
	  "time off\n",
	  "cpd ${obsid}_${runname}_spectrum.ps/vcps\n",
	  "plot\n",
	  "cpd /null\n",
	  "exit\n";
      ...
      myprint 'set fileid [open '.${obsid}.'_'.${runname}.'_lumin.dat w]'."\n";
	  ...
	  myprint "tclout pinfo $par\n";
	  myprint 'scan $xspec_tclout "%s" name'."\n";
	  myprint "tclout param $par\n";
	  myprint 'scan $xspec_tclout "%f" bfpar'."\n";
	  myprint "tclout error $par\n";
	  myprint 'scan $xspec_tclout "%f %f" lopar hipar'."\n";
	  myprint 'eval puts \$fileid \"$name '."$par".' $bfpar $lopar $hipar"'."\n";
      }
      myprint $filler;
  ...
  system("mv -f ${obsid}_${runname}_spectrum.ps $specdir");
}
...
#    } else {
	printf FITFILE "%-25s %6s %6.3f %6.3f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f %10.2e %10.2e %10.2e %5.2f %5.2f %5.2f %10.2e %10.2e %10.2e %7.4f %7.4f %7.4f %6.3f %6s %6.2f %6i\n",
	$name,$obsid,$rin,$rout,$nh*1e2,$nlo*1e2,$nhi*1e2,$tx,$tlo,$thi,$fe,$felo,$fehi,$norm,$normlo,$normhi,$tx2,$tlo2,$thi2,$norm2,$normlo2,$normhi2,$z,$zlo,$zhi,$cr,$ptot,$stats[0],$dof[0];
#    }
  close FITFILE
}
