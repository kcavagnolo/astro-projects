...
const double nullval = 1.320475e-29;
...
void CFITSStack::Log()
{
  ...
  for(int x = image.GetXW()-1; x >= 0; x--)
	...
	cerr << "*  log of negative value attempted\n";
      }
    }
...
void CFITSStack::Sqrt()
{
  ...
  for(int x = image.GetXW()-1; x >= 0; x--)
	...
	cerr << "*  sqrt of negative value attempted\n";
      }
    }
...
{
  cout << "FITSMaths, Jeremy Sanders, 2000, 2001\n";
}
...
void CFITSMathProgram::errormessage()
{
  cerr <<
    ...
    "operator can be +,-,/,*,dup,ln,log,sqrt,exp,drop,swap,lt,gt,ltroll,gtroll\n";
  exit(-1);
}
