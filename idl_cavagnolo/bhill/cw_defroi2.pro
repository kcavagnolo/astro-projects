;
; $Id: cw_defroi2.pro,v 1.1.1.1 2007-02-03 17:10:49 cavagnolo Exp $
; LOCAL VARIANT:  cw_defroi2, 5 Oct. 1999 - RSH, RITSS
;
...
PRO CW_DEFROI2_DRAW, s, i, FILL = fill
...
ENDIF ELSE BEGIN            ;Polygon/circle/rect
    if n ge 2 then begin
        if i lt 0 then plots, COLOR=col, $
            xy[*, 0:n-1]+.5 $ ;All of it?
...
PRO CW_DEFROI2_event, ev, s
; This routine is only called from the CW_DEFROI2 event loop.
; ev = event structure, s = state structure.


IF tag_names(ev,/struct) NE 'WIDGET_DRAW' THEN RETURN
...
if (s.mode eq 2) or (s.mode eq 3) then begin ;Rect or square?
    if s.button ne 0 then begin          ;Drag
        if n gt 0 then CW_DEFROI2_draw, s, -1      ;Remove old
        t = s.drag
...
if tmode then begin            ;Remove prev point?
    if (ev.press ne 0) and (n gt 0) then begin
        CW_DEFROI2_DRAW, s, -1   ;Erase old region
        ...
        if n gt 1 then CW_DEFROI2_DRAW, s, -1   ;Draw new region
        endif   
    return
    endif               ;Remove mode....   
...
if s.mode eq 0 then begin           ;Polygon?
    if n ge 2 then CW_DEFROI2_draw, s, n-2           ;Draw the new segment
endif else begin                    ;Point
    CW_DEFROI2_draw, s, n-1         ;Draw new point
endelse
return
...
function CW_DEFROI2, draw, ZOOM = zoom, IMAGE_SIZE = image_size, $   
    OFFSET = offset, RESTORE = restore, ORDER = order, $
    GROUP_LEADER = group_leader, FLOATING = floating
...
WHILE 1 DO BEGIN                ;Internal event loop   
    ...
    n = s.npts
    if ev.id eq draw then CW_DEFROI2_EVENT, ev, s $
...
'Clear':  BEGIN   
    if (n ge 2) or (s.mode eq 1 and n ge 1) then $
        CW_DEFROI2_draw, s, -1        ;Erase roi   
    s.npts = 0   
    CW_DEFROI2_NMODE, s, s.mode
    ENDCASE
'New' : CW_DEFROI2_nmode, s, s.mode      ;Make a new region...
'Cancel':  BEGIN   
    xy = -1
    goto, all_done
    ENDCASE    
...
;    ['Polygon', 'Point', 'Rectangle', 'Circle'], SET_VALUE=0)
'Polygon': CW_DEFROI2_nmode, s, 0
'Point' :  CW_DEFROI2_nmode, s, 1
'Rectangle' :  CW_DEFROI2_nmode, s, 2
'Circle' :  CW_DEFROI2_nmode, s, 3

'Add':  s.amode = 0
'Remove': s.amode = 1
'Done': BEGIN   
    cw_DEFROI2_nmode, s, 0       ;Save old region
    WIDGET_CONTROL, s.subs, GET_UVALUE=t, /NO_COPY  ;List of subscripts
    xy = BYTARR(s.image_size[0], s.image_size[1])  ;Return only unique
    ; t can be -1 if no points are selected (don't index into xy with t)
    ; t can be an array of a single point in the case of point mode
    ; t can be an array of many elements in the general cases (polygon/rect) 
    if ((n_elements(t) gt 1) or ((n_elements(t) eq 1) and (t[0] ge 0))) then $ 
       xy[t] = 1
