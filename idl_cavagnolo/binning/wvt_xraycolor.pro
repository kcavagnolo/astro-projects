...
;
; INPUTS (REQUIRED):
;        SIGNAL: Two-dimensional image (n_x,n_y), containing the signal per pixel
...
;    BINNUMBER: Two-dimensional image (n_x,n_y) containing the bin number assigned
;               to each input pixel. The index goes from 1 to Nbins. Pixels that
;               were excluded during the binning process are marked "0" (see 
;               also the keyword MASK).
...
;
; KEYWORDS (OPTIONAL):
;       CENTER: [Input] Vector (size 2) containing x and y values for the center. 
...
;               start there. 
;     MAX_AREA: [Input] Scalar specifying a maximum bin size (in pixel). Useful 
;               in cases where there is essentially no signal in a certain region.
;               ATTENTION: In area where the bin size hits MAX_AREA, the 
;               algorithm does NOT enforce a uniform S/N anymore. Use with care. 
;         MASK: [Input] Two-dimensional image (n_x,n_y) specifying which pixels 
;               should be included in the WVT binning algorithm. Valid pixels 
;               are designated as "1", excluded pixels as "0" (integer or byte). 
...
;               information to restart WVT_XRAYCOLOR from any given point. If the
;               keyword RESUME is set, the WVT iteration scheme will be applied 
;               to the supplied WVT.
;
...
COMMON DATAVALUES, P
  IF total(P.signal2[index]) EQ 0 or total(P.cts[index]) EQ 0 THEN RETURN, 0d0 $
...
PRO WVT_XRAYCOLOR, signal, signal2, noise, noise2, targetSN, binnedimage, $
  ...
  ; Create the structure save_all, which can be used to resume the procedure at 
  ; the point it was stopped. The structure can also be used to start with a 
  ; given tesselation
  save_all={ x:x, $
             y:y, $
             class:class, $
