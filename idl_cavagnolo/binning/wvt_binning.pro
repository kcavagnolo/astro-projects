...
; Version 1.9.3 updated 09/15/05
;
;######################################################################
...
;
; EXPLANATION:
;       For additional information about weighted voronoi binning (WVT binning),
...
;
; REQUIREMENTS:
;       There have to be two external, compiled functions ADD_SIGNAL and ADD_NOISE,
;       which define how the signal and noise inside a bin are added. See WVT_IMAGE
;       for an example.
;
; INPUTS (REQUIRED):
...
;               images, a temperature map would require higher TARGETSN (~30-100), 
;               depending on the spectral model used. For integral field 
;               spectroscopy, a TARGETSN of ~50 per pixel may be a
...
;               The image can be background subtracted or exposure map corrected,
;               as long as the NOISE image reflects this. 
...
;            WVT_RENUMBER_BINNUMBER        -- kick out zero sized bins and renumber
;            WVT_UNWEIGHTED_CENTROID       -- calculate geometric center
;            WVT_ADDTO_UNWEIGHTED_CENTROID -- update center when pixels are added
...
;            WVT_MAKENEIGHBORLIST          -- create list of neighbors for pixels
...
;            WVT_RECURSIVE_NEIGHBORS       -- get list of nearby bins recursively
...
;           (11/2004)
;       V1.1: Added stability checks, redistribution of "bin islands" (12/2004) 
...
PRO WVT_CHECK_BINISLANDS, x, y, binnumber, neighborbinnumber, xnode, ynode, $
  ...
  ; Checks that every bin is not enclosed by another bin, i.e. has more than one
  ; neighboring bin. Note that this check should be avoided if you very gaps in
  ; your data since then you could have an isolated bin due to the fact that the
  ; gaps are "cornering" the bin.
  ...
  FOR k=0L,nbins-1 DO BEGIN
    ...
    IF nneighbors EQ 1 THEN BEGIN
      ...
      FOR i=0L,n_elements(index)-1L DO $
          binnumber[index[i]]=temp[wvt_assign_to_bin(x[index[i]], y[index[i]], $
...
;----------------------------------------------------------------------------
PRO WVT_ADDTO_WEIGHTED_CENTROID, x, y, xymass, xBar, yBar, xbold, ybold, massold
  ; For speed, this procedure computes the geometric center of a bin by adding 
  ; a new list of xy values for an existing bin. Useful in bin accretion step. 
...
PRO WVT_BIN_ACCRETION, x, y, dens, targetSN, binnumber, pixelSize, $
  ...
  FOR ind=1L,n do BEGIN
    ...
    WHILE 1 DO BEGIN
      ...
      ; Use the average bin members' S/N to estimate the S/N when another pixel 
      ; is added to see if adding another pixels would actually increase the 
      ; scatter around the target S/N due to "overshooting". Also stop accreting 
      ; if the bin size reached "max_area"
      ...
      ; For speed, remember the neighbors from the last accretion step and simply
      ; add the new ones.
...
;          xBarnew, yBarnew, xbar, ybar, area 
    ...
    IF ind GE nfixed THEN $
      minDist = min((x[unBinned]-xBar_binned)^2 + (y[unBinned]-yBar_binned)^2,k)$
  ...
  binnumber = binnumber * good ; Set all bins to zero that did not reach the target S/N

END

;----------------------------------------------------------------------------
PRO WVT_REASSIGN_BAD_BINS, x, y, dens, targetSN, binnumber, xnode, ynode, SNBin=SNBin
  ...
  FOR j=0L,m-1 DO $
    binnumber[bad[j]] = good[wvt_assign_to_bin(x[bad[j]],y[bad[j]],xnode, ynode,1d0)]+1
...
;      xnode=(xnode+xnodeold)/2.
;      ynode=(ynode+ynodeold)/2.
;    ENDIF 
    IF pixelswitch/double(npixels) LT .01 AND iter GT 20 THEN BEGIN
...
;      IF pixelswitch/double(npixels) GT .01 OR iter LT 2 THEN $
      IF pixelswitch/double(npixels) GE .01 OR iter LE 20 THEN $
        SNbinbar=SNbin/double(binarea) $
      ELSE BEGIN
;        SNbinbar=(SNbinold+SNbin/double(binarea))/2.
        temp=randomu(seed, n_elements(xnode))
        wh=where(temp GT .5, nwh)
        IF nwh NE 0 THEN SNbinbar[wh]=(SNbinold[wh]+SNbin[wh]/double(binarea[wh]))/2.
      ENDELSE

      SNbinold=SNbinbar
      ; Prepare for the case that s/n in a bin is either 0 or negative
      ; CHECK THIS OUT AGAIN!	
  ;      SNmin=(0.9*min(SNbinbar(where(SNbinbar GT 0))))>SNmin_area; restrict size
  ;      SNbinbar=SNbinbar>SNmin
    ...
    wvt_find_binneighbors, binneighbors, neighborlist, neighborbinnumber, binnumber, $
      nbins, area=area, reverse_indices=r
    FOR k=0L,nbins-1 DO BEGIN 
  ...
  ENDIF
    ...
    FOR j=0L,m-1 DO BEGIN
      ...
      IF j GE nfixed THEN BEGIN    
        ...
        ELSE $
          wvt_unweighted_centroid, x[index], y[index], xb, yb  ; To get final right
      ENDIF ELSE BEGIN
        xb=keepfixed[0,j]
        yb=keepfixed[1,j]
      ENDELSE
    ...
    ;Now make sure that still each bin has at least one pixel 
  ;  wvt_check_all_bins, binnumber, x, y, xnode, ynode, area=area, reverse_indices=r

    ; Redistribute enclosed bins ("bin islands")
    wvt_check_binislands,  x, y, binnumber, neighborbinnumber, xnode, ynode, area=area, $
    reverse_indices=r
    ...
    ENDIF
    iter = iter + 1
  ; Stop the iteration if less than .25% of all pixels stop switching bins, 
  ; you hit the maximum number of allowed iterations, or if the program is 
  ; terminated externally by the creating the control file "stopmenow".
  area = HISTOGRAM(binnumber, REVERSE_INDICES=r) 
  w1 = where(area LE 2 OR area GT max_area*.95, COMPLEMENT=w2, m)
  IF n_elements(w2) GT 3 THEN snwidth=stddev(snbin[w2]-avg(snbin[w2])) $
    ELSE snwidth=-1
;  printf, 99, pixelswitch, diff, snwidth
...
;  print, iter, pixelswitch, psavg, oldpsavg
  ...
  IF iter GT max_iter THEN $
    print, "Iteration reached maximum number of iterations. This should not happen in general except for data with an extremely large dynamical range. Check your input files and/or restart the iteration from this point with the save_all and resume keywords."

  IF pixelswitch EQ 0. THEN $
    print, "Iteration converged to a stable WVT solution"
  IF (psavg GE oldpsavg AND iter GT 10+navg) THEN $
    print, "Changes per iteration have flattened out. The results are very unlikely to change from this point on. If you are not satisfied with the solution, restart from this point with the save_all and resume keywords." 
...
PRO WVT_FINDNEIGHBORS, group, neighborlist, neighbors, exneighbors, newmember
  ...
  ENDIF ELSE BEGIN
    ...
    ; First check which neighbors of the new member are already part of 
    ...
    ;help, neighbors
    ;subneighbors=where(exneighbors[group])
    ;neighbors=where(exneighbors eq 1)
    ;match, tmp, group, subneighbor, subgroup
    ;exneighbors[group]=0
    ;neighbors=where(exneighbors eq 1)
  ENDELSE
  ;neighbors=neighbors(uniq(neighbors, sort(neighbors)))
  ...
  ;wh=where(neighbors ne -1)
  ;IF wh[0] EQ -1 THEN neighbors=-1 ELSE neighbors=neighbors(wh)
  ; Check if the neighbors of the the new 
  ;IF n_elements(neighbors) GT 1 THEN neighbors=neighbors(sort(neighbors))
END
