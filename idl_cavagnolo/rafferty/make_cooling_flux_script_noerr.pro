pro make_cooling_flux_script_noerr,nregions,ncool,nH_gal,kT_low,kT_high,ab_guess, $
          redshift,lo_energy,hi_energy,bin,inner_a,inner_b, $
          inner_pa
;-----------------------------------------------------------------------
;
; Name: MAKE_COOLING_FLUX_SCRIPT
;
; Purpose: Creates an XSPEC script to deproject the spectra extracted by
;	   EXTRACT_ANNULI with the projct*wabs*(mekal+mkcflow) model and to 
...
;
; Read in best-fit parameters from previous mkcflow fit
;
infitsfile='source_mkcflow.spectra'
...
;
get_lun,unit
openw,unit,'projct_mkcflow_flux.xcm'
printf,unit,'# XSPEC script for finding PROJCT*WABS*(MEKAL+MKCFLOW)
printf,unit,'# flux inside the cooling radius.'
...
;
printf,unit,' '
printf,unit,'set model_name "projct_mkcflow_flux"'
...
printf,unit,'set fitsfl "source_mkcflow_flux.spectra"'
...
printf,unit,'set red_chi_squared [expr $chi_squared/$dof_value]'
printf,unit,' '
printf,unit,'# output info to xspec_mkcflow_flux.log'
...
printf,unit,'file copy -force z_temp_xspec.log xspec_mkcflow_flux.log'
...
printf,unit,'# Set mkcflow norms to best-fit values'
for i=0,ncool-1 do begin
   printf,unit,'newpar '+strtrim(string(16+i*16),2)+' '+strtrim(string(cnorm[i]),2)
endfor
printf,unit,'# Calculate best-fit flux'
printf,unit,'  if {$ii < $BandNum} {'
printf,unit,'    flux [lindex $EbandLo $ii] [lindex $EbandHi $ii]'
printf,unit,'  } else {'
printf,unit,'    set iiii [expr $ii-$BandNum]'
printf,unit,'    flux [lindex $EbandLo $iiii] [lindex $EbandHi $iiii]'
printf,unit,'  }'
for i=1,ncool do begin
   printf,unit,'  tclout flux '+strtrim(string(i),2)
   printf,unit,'  scan $xspec_tclout "%f" Ftemp'+strtrim(string(i),2)
endfor
printf,unit,'# Set mkcflow norms to upper values'
for i=0,ncool-1 do begin
   printf,unit,'newpar '+strtrim(string(16+i*16),2)+' '+strtrim(string(cnorm_hi[i]),2)
endfor
printf,unit,'# Calculate best-fit flux'
printf,unit,'  if {$ii < $BandNum} {'
printf,unit,'    flux [lindex $EbandLo $ii] [lindex $EbandHi $ii]'
...
for i=1,ncool do begin
   printf,unit,'  tclout flux '+strtrim(string(i),2)
   printf,unit,'  scan $xspec_tclout "%f" Ftemp'+strtrim(string(i),2)+'hi'
endfor
printf,unit,'# Set mkcflow norms to lower values'
for i=0,ncool-1 do begin
   printf,unit,'newpar '+strtrim(string(16+i*16),2)+' '+strtrim(string(cnorm_lo[i]),2)
endfor
printf,unit,'# Calculate best-fit flux'
printf,unit,'  if {$ii < $BandNum} {'
printf,unit,'    flux [lindex $EbandLo $ii] [lindex $EbandHi $ii]'
...
for i=1,ncool do begin
   printf,unit,'  tclout flux '+strtrim(string(i),2)
   printf,unit,'  scan $xspec_tclout "%f" Ftemp'+strtrim(string(i),2)+'lo'
endfor
...
printf,unit,'}'
printf,unit,'# add flux and error info to xspec_mkcflow_flux.log'
printf,unit,'set fileid [open xspec_mkcflow_flux.log a]'
printf,unit,'for {set ii 0} {$ii <=[expr $BandNum-1]} {incr ii} {'
printf,unit,'  if {$ii < $BandNum} {'
