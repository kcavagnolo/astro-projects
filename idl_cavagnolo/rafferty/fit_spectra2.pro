...
;	    /XRAY_FLUX - if set, the X-ray flux inside the cooling radius is 
;		            calculated (must run EXTRACT_COOLING first)
;         
;         
; Comments: 
;           
...
if (np gt 0) then begin
   ...
   'fit_spectra [, /PROJECTED, /DEPROJECTED, /COOLING, /FLUX, /XRAY_FLUX, /CENTRAL, /OLD_PIPE, spectra_dir=spectra_dir]'
   return   
endif
if (keyword_set(cooling) or keyword_set(projected) or $
    keyword_set(deprojected) or keyword_set(central) or $
    keyword_set(xray_flux)) then begin
endif else begin
   deprojected=1
endelse
...
;
paramfile=findfile('fit.par',count=num)


;
; If found, read in last parameters
;
if (num eq 1) then begin
   readcol,paramfile,nreg0,nH_gal0,kT0,ab0,redshift0,lo_energy0,hi_energy0,bin0,ncool0,kt_low0
endif else begin
   nreg0=4
   nH_gal0=0.02
   kT0=3.0
...
print,' '
read,nreg,prompt='       Number of regions to fit ('+fix(nreg0)+'): '
if (keyword_set(cooling) or keyword_set(xray_flux)) then read,ncool,prompt='          Cooling region number ('+fix(ncool0)+'): '
read,nH_gal,prompt='      Galactic nH [10^22 cm^-2] ('+nH_gal0+'): '
read,kT_guess,prompt='         kT initial guess [keV] ('+kT0+'): '
if keyword_set(cooling) then read,kT_low,prompt='     Cooling model low kT [keV] ('++'): '
...
if (keyword_set(cooling) or keyword_set(xray_flux)) then print,fmt0,'12)   Cooling region number = ',ncool
if keyword_set(cooling) then print,fmt1,'13)    Cooling model low kT = ',kT_low, ' [keV]        '
print,'-------------------------------------------------'
print,' '
answer=' '
...
if (answer eq 'n') then begin
   ...
   if keyword_set(xray_flux) then begin
      read,par_to_correct,prompt='Enter number of parameter to correct (1-12): '
   endif else begin
   ...
   if keyword_set(xray_flux) then begin
      if ( (par_to_correct lt 1) or (par_to_correct gt 12) ) then begin
         read,par_to_correct,prompt='Please enter a number between 1 and 12: '
         goto,par_check
      endif
   endif else begin
...
if (binning eq 1) then begin
   ...
   if (keyword_set(cooling) or keyword_set(xray_flux)) then begin
...
;
; If /XRAY_FLUX keyword set, generate XSPEC script
;
if keyword_set(xray_flux) then begin

   print,' '
   print,'Now generating XSPEC script...'
   make_cooling_flux_script,nreg,ncool,nH_gal,kT_guess,ab_guess,redshift,lo_energy,hi_energy,binning,inner_a,inner_b,inner_pa
...
if keyword_set(xray_flux) then append_text=append_text+', /XRAY_FLUX'
