...
;           Use a single mag factor when loading maps.
...
;-------------------------------------------------------------
	pro screenpng, file, window=outwin, error=err, image=img, help=hlp, $
	  mag=mag, pixmap=pixmap, rebin=rebin, iwin=iwin, info=info, $
	  noswin=noswin
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     Use a single mag factor when loading maps.'
	...
	set_scale, image=img0, /quiet  	     	; Set scale.
