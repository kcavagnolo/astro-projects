...
;-------------------------------------------------------------
	...
	if n_elements(set) ne 0 then file=set+'.event_log' ; Given filename.
	name = filename(dir,file,/nosym)	; Add directory.
...
help:	  print,' Enter an event into the latest event log file.'
	...
	if n_elements(dir) eq 0 then dir=''	; Default to current directory.
	...
	if n_elements(set) gt 0 then begin	; Given event file name.
	  name = set+'.event_log'
	  file = filename(dir,name,/nosym)	; Deal with directory.
