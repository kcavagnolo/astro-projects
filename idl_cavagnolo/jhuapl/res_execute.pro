...
;-------------------------------------------------------------
	...
	for i=0, n_elements(code)-1 do begin		; Loop through lines.
	  ...
	  if w1 eq 'CODE_BLOCK' then begin		; Was it a code block?
	    ...
	    res_execute_addcode, code2, clist		; Add new block to list.
	  endif else begin
...
;-------------------------------------------------------------
	...
	if nvar gt 0 then begin		; There were some.
	  ...
	  for i=0,nvar-1 do err=execute(etags(i)+'=extra.(i)')
	endif
	...
	res_execute_addcode, code, clist	; Add code to clist.
 
	;---------------------------------------
	;  Execute IDL code
	;---------------------------------------
	for i=0, n_elements(clist)-1 do begin	; Execute complete code list.
	  err=execute(clist(i))
	endfor
