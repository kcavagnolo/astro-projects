...
;         keyword.  If FD=fd is not used then the last res file
;         accessed, if any, will be used.  This may cause an error,
;         so if fd is used at all use it for all calls in the idl
;         session.
...
;         to be in use.  Pass fd to a routine through the FD
...
;-------------------------------------------------------------
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'   to be in use.  Pass fd to a routine through the FD'
	  print,'   keyword.  If FD=fd is not used then the last res file'
	  print,'   accessed, if any, will be used.  This may cause an error,'
	  print,'   so if fd is used at all use it for all calls in the idl'
	  print,'   session.'
   	  return
	end
