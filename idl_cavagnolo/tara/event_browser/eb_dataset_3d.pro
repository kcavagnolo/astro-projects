...
;========================================================================
;;;
;;; FILE NAME:    $Id: eb_dataset_3d.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
;;; DESCRIPTION:  Trivariate Analysis Widget for Event Browser
;;;
...
;==========================================================================
PRO EbDataset3dCleanup, top_base
...
;==========================================================================
FUNCTION EbDataset3dDisplayEvent, event
...
case event.id of
  ...
  state.z_property: $
    begin 
    state.zprop_name = state.z_list[event.index]
    state.new_property_flag = 1
    state.dds_revised       = 1
    state.relabel_flag      = 1
    end
...
if (state.new_property_flag) then begin
  ...
  widget_control, state.dataset_3d,    SENSITIVE=0, BAD_ID=bad_id
...
if (redisplay_flag) then begin
  ...
  state.vlabel = vlabel
  z=GetProperty( state.zprop_name, WDS_DATA=zdata, LABEL=zlabel )
    
  descrip = state.dataset_name 
  ...
  if (state.new_property_flag) then begin
    ...
    dataset_3d, state.dataset_3d, DATASET_NAME=state.dataset_name, $
    		REDRAW=0, XBIN=0, YBIN=0
  endif
  
  if (state.relabel_flag) then begin 
    state.relabel_flag = 0   
    dataset_3d, state.dataset_3d, hdata, vdata, zdata, $
		 XTITLE=hlabel, YTITLE=vlabel, ZTITLE=zlabel,$
		 XWCS=hwcs, YWCS=vwcs, $
		 UNITY_ASPECT=(state.hprop_name EQ 'x_position') AND $
		 	      (state.vprop_name EQ 'y_position'), $
		 DATASET_NAME=state.dataset_name, DESCRIPT=descrip 
  endif else begin
    dataset_3d, state.dataset_3d, hdata, vdata, zdata, $
		 DATASET_NAME=state.dataset_name, DESCRIPT=descrip 
  endelse
...
;==========================================================================
FUNCTION GetEbDataset3dValue, top_base
...
widget_control, top_base, GET_UVALUE=state, /NO_COPY
  ...
  dataset_3d, state.dataset_3d, DATASET_NAME='domain dataset', ROI_MASK=roi_mask
...
;========================================================================
FUNCTION eb_dataset_3d, manager_widget, $
	 dataset_name, hprop_name, vprop_name, zprop_name, $
	 STAT_CODE=stat_code, $
	 UVALUE=uvalue, TITLE=title, GROUP=group, GENERIC=generic

if NOT keyword_set(uvalue) then uvalue = 0
if NOT keyword_set(title)  then title  = 'Trivariate Analysis'
if NOT keyword_set(group)  then group  = 0
if (n_elements(stat_code)  EQ 0) then stat_code  = -1
...
top_base = widget_base( parent, /COLUMN, $
			FUNC_GET_VALUE='GetEbDataset3dValue', $
			EVENT_FUNC='EbDataset3dDisplayEvent', $
			KILL_NOTIFY='EbDataset3dCleanup')

 dataset_3d_base = widget_base( top_base, /SPACE, XPAD=0, YPAD=0 )

 if (stat_code GE 0) then begin
   dataset_3d, dataset_3d, PARENT_WIDGET=dataset_3d_base, STAT_CODE=stat_code
 endif else begin
   dataset_3d, dataset_3d, PARENT_WIDGET=dataset_3d_base
 endelse

 bottom_base = widget_base( top_base, /ROW, /SPACE,XPAD=0,YPAD=0, /ALIGN_LEFT )
 ...
 if keyword_set(generic) then begin
   z_list = [GetPropertyNames( /FITS, DIM=1, /NUMERIC_ONLY ),'timestamp','row_number']
 endif else begin
   z_list = ['exposure', 'timestamp', 'energy', 'wavelength', 'pha', 'instrument_grade', 'asca_grade', 'generic1', 'generic2', 'generic3','generic4', 'generic5', 'generic6']
 endelse

  vertical_property = $
	widget_droplist( left_base, VALUE=prop_list, TITLE='Vertical' )

  z_property = widget_droplist( left_base, VALUE=z_list, $
  				TITLE='Animation or Statistic' )
  ...
  z_index          = (where(z_list    EQ zprop_name, count))[0]
  if (count EQ 0) then begin
    z_index        = 2 < (n_elements(z_list)-1)
    zprop_name = z_list[z_index]
  endif
  ...
  widget_control, z_property,          SET_DROPLIST_SELECT=z_index
...
state = { parent:parent, $
	...
	top_base:top_base, dataset_3d_base:dataset_3d_base,$
	dataset_3d:dataset_3d, rot_angle:rot_angle, $
	prop_list:prop_list, z_list:z_list, $
	hprop_name:hprop_name, vprop_name:vprop_name, zprop_name:zprop_name,$
	...
	; Other stuff
	hlabel:'', vlabel:'', stat_code:stat_code, $
...
xmanager, 'EbDataset3d', parent, GROUP_LEADER=group, $
