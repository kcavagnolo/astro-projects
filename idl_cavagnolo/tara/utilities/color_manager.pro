...
;========================================================================
;;;
;;; FILE NAME:    $Id: color_manager.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
...
if (n_elements(model) EQ 0) then begin
  print, 'Initializing the TARA color manager v$Revision: 1.2 $ ...'
  ...
  ; top of the table.
  if (NOT keyword_set(num_reserved)) then num_reserved = 5
  n_reserve = 5 > num_reserved < 9
  
  rc = replicate({name:'', r:0, g:0, b:0}, 9)
  
  rc.name = ['yellow','light blue','purple','orange','blue','green','red','black','white']
  rc.r    = [    255,          0,     255,     255,     0,      0,    255,      0,   255 ]
  rc.g    = [    255,        255,       0,     128,     0,    255,      0,      0,   255 ]
  rc.b    = [      0,        255,     255,       0,   255,      0,      0,      0,   255 ]
  
  reserved_colors = rc[9-n_reserve:*]
...
if (0 NE n_elements(extra)) then device, _EXTRA=extra
  
ncolors         = (*model).ncolors
negative_images = (*model).negative_images
decomposed      = (*model).decomposed

if arg_present(red)     then red   = ((*model).reserved_indexes)[i_red]
if arg_present(green)   then green = ((*model).reserved_indexes)[i_green]
if arg_present(blue)    then blue  = ((*model).reserved_indexes)[i_blue]
if arg_present(white)   then white = ((*model).reserved_indexes)[i_white]
if arg_present(black)   then black = ((*model).reserved_indexes)[i_black]
...
if keyword_set(color_name) then begin
  ...
  if (count NE 1) then message, 'Invalid color name: ' + color_name
  
  color_index = ((*model).reserved_indexes)[index[0]]
endif
