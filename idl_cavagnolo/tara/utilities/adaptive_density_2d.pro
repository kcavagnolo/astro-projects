;;; $Id: adaptive_density_2d.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
...
;;; The input data "image" must be integer counts (so that Poisson statistics can be used).
...
PRO adaptive_density_2d, image, min_significance_param, $
			EMAP=emap_param, BACKGROUND_MAP=background_map, FIELD_MASK=field_mask, $
			MAX_RADIUS=max_radius, RADII=radii, FIXED_RADIUS_MAP=fixed_radius_map,$
...
if keyword_set(fixed_radius_map) then begin
  ...
  if NOT keyword_set(max_radius) then max_radius=ceil( (xdim>ydim)/2.0 )
...
if NOT keyword_set(background_map) then background_map = replicate(0, xdim, ydim)

if NOT keyword_set(field_mask)     then field_mask     = replicate(1B, xdim, ydim)
...
if keyword_set(emap_param) then begin
  ...
  if (count GT 0) then begin
    ...
    if (count GT 0) then message, 'ERROR: data found where exposure is <= zero!'
    
    dum = where(background_map[ind] NE 0, count)
    if (count GT 0) then message, 'ERROR: background found where exposure is <= zero!'
  endif
...
checkpoint_rows = round([.1,.2,.3,.4,.5,.6,.7,.8,.9]*ydim) > 1
...
for y = 0,ydim-1 do begin
  ...
  for x = 0,xdim-1 do begin
    ...
    ;; In this algorithm "flux_estimate" has so far been a non-negative observed
    ;; flux estimate (counts under the kernel normalized by exposure under the kernel).
    ;; This non-negative flux was used as a poisson mean in the equations that 
    ;; estimate significance.
    ;; Now, at the end, we finally subtract a normalized background under the kernel.
    ;; The statistical assumption here is that there is little Poisson uncertainty in
    ;; the background map.
    background = total(weight * background_map[pixel_list])
    flux_map  [x,y] = flux_estimate - (background / exposure)
