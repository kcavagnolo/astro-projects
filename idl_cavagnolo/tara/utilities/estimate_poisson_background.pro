;;; $Id: estimate_poisson_background.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
...
if NOT keyword_set(significance) then significance = 0.99
  ...
  for dum = 1, 1000 do begin
    ...
    if keyword_set(verbose) then begin
       print, bkg_flux, 100*(1 - num_good/float(num_data)), $
           F='(%"background flux=%g; %5.1f%% of data discarded ")'
    endif
...
;help, confidence_limit  
  ...
  if keyword_set(verbose) then begin
    print, mean_bkg_counts, 100*(1 - num_good/float(num_data)), max_val, $
           F='(%"background=%g; %5.1f%% of data discarded above %f")'
  endif
