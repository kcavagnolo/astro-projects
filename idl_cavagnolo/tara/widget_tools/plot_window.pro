...
;========================================================================
;;;
;;; Plot Window Widget: $Id: plot_window.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
...
; 
  (*st).edit: $
   ...
   f1 = '0, FLOAT,' + string((*st).draw_size[0]) + $
	 ', TAG=xdim, LABEL_LEFT=X, WIDTH=12'

   f2 = '2, FLOAT,' + string((*st).draw_size[1]) + $
   ...
   draw_size = [result.xdim, result.ydim] > [200,200] < [1300,1000]
...
if keyword_set(parent) then begin
  ...
  down_base = widget_base(top_base, /ROW, /SPACE, /XPAD, /YPAD )
    wcs_name  = 0L  ;widget_label( down_base, VALUE=' ', /DYNAMIC_RESIZE )
    wcs_value = widget_label( down_base, XSIZE=210, VALUE=' ' )
  ...
  wcs_object, wcs, /INIT
  axis = { default_flag:1, log_flag:0, range:[0.0,0.0], add_margin:0, $
...
if keyword_set(change_draw_size) then begin
  (*st).draw_size = (*st).draw_size + change_draw_size
...
if keyword_set( show_axes ) then begin
  ...
  plot, [0,1], /NODATA, XRANGE=xrange, YRANGE=yrange, XSTYLE=3, YSTYLE=3, $
