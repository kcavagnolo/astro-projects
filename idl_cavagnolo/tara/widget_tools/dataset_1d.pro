...
;========================================================================
;;;
;;; Dataset_1D Widget: $Id: dataset_1d.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
...
;		 XTITLE=xtitle, XWCS=xwcs, NAN_VALUES=nanval,$
...
dataset = { index:0B, name:'', description:'', metric:0.0, $
	    ...
	    plot: {color:'white', psym:3B, line:0, show_errors:0, error_skip:4}, $
...
state = { parent:parent, $ 
	...
	normalize_density:1, $
	norm_absc:ptr_new(/ALLOC), norm_val:ptr_new(/ALLOC),$
	...
	density_widget:0L, samples_widget:0L, ps_config:ptr_new(/ALLOC) }
	
	  
;; Allocate heap variables in dataset structures.
color=['white','green','blue','red'] 	& nc = n_elements(color)
line =[0,1]				& nl = n_elements(line)
for ii = 0, n_elements(state.datasets)-1 do begin
  ...
  state.datasets[ii].plot.color = color[(ii/2) mod nc]
  state.datasets[ii].plot.line  = line [ ii    mod nl]
endfor
...
ptr_free, (*st).datasets.x_data, (*st).datasets.x_filtered_data, $
	  ...
	  (*st).datasets.fit.samples,(*st).norm_absc, (*st).norm_val, $
	  (*st).ps_config
ptr_free, st
return
end
...
if (dataset.stats_stale) then begin
  ...
  if (num_points EQ 0) then begin
    ...
    if (n_elements( *(*st).norm_absc ) EQ 0) then begin
...
if (n_elements( *(*st).norm_absc ) GT 0) then begin
  bin_center = d.x0 + bin.delta_x * findgen(d.xdim)
  
  norm = interpol( *(*st).norm_val, *(*st).norm_absc,  bin_center )
...
case (mode) of
 ...
 ;-------------------------------------------------------------------------
  ...
  if (lstyle NE 0) then begin
        ...
        legend, plotsets.description, PSYM=plotsets.plot.psym,  $
 ...
 ;-------------------------------------------------------------------------
  ...
  if ((*st).stale_titles) then begin
    ...
    case (*st).normalize_density of
     0: ytitle = 'Counts'
     1: ytitle = 'Data Density (# per X-unit)'
  ...
  for ii = 0, n_elements(plotsets)-1 do begin
    ...
    if (p.show_errors AND n_elements(*d.error) GT 1) then begin
      error = [0.,*d.error,0.]
      oploterror, bin_center, bin_value, error, PSYM=10, LINESTYLE=p.line, ERRSTYLE=0, COLOR=plot_color, ERRCOLOR=plot_color, NSKIP=p.error_skip, /NOHAT
    endif else begin
      oplot, bin_center, bin_value, PSYM=10, LINESTYLE=p.line, COLOR=plot_color
    endelse
  ...
  if (lstyle NE 0) then begin
        ...
        legend, plotsets.description, LINE=plotsets.plot.line,  $
                /TOP,LEFT=(lstyle EQ 1),CEN=(lstyle EQ 2),RIGHT=(lstyle EQ 3),$
                COLORS=colors, TEXTCOLORS=colors, CHARSIZE=0.75
  endif
  end
 ...
 ;-------------------------------------------------------------------------
  ...
  for ii = 0, n_elements(plotsets)-1 do begin
    ...
    oplot, bin_center, *distn.samples, LINE=p.line, COLOR=plot_color
    ...
    2: $ ; SAVE SELECTED DENSITY
     ...
     if (pathname NE '') then begin
       ...
       if (Error NE 0) then begin
         ...
         sxaddpar, pheader, "CREATOR", "$RCSfile: dataset_1d.pro,v $, Version $Revision: 1.2 $"
	 ...
	 writefits, pathname, no_data, pheader
         ...
         sxaddpar, fits_header, "CREATOR", "$RCSfile: dataset_1d.pro,v $, Version $Revision: 1.2 $"
...
; Edit button: $
  (*st).edit_button: $
   ...
   ;  PLOT PROPERTIES
    ...
    case (selected_dataset.plot.color) of
     'red'  : color_index = '0'
     'green': color_index = '1'
     'blue' : color_index = '2'
     'white': color_index = '3'
    ...
    f2 = '0, BUTTON, Red|Green|Blue|White,'+$
    ...
    form = [form,b0,l0,f1,f4,f2,b1,f5,f6,f3]
  ...
  if (mode NE 0) then begin
   ...
   case (r.color) of
       0: selected_dataset.plot.color = 'red'
       1: selected_dataset.plot.color = 'green'
       2: selected_dataset.plot.color = 'blue'
       3: selected_dataset.plot.color = 'white'
   endcase
...
if (create_flag) then top_base = CreateDataset1d( _EXTRA=extra )
...
if (0 NE n_elements(title))    then begin
  print, '!!!!!!!!!!!!!!!!!!!'
  print, 'Obsolete keyword TITLE used in dataset_1d call; please notify Patrick Broos'
  print, '!!!!!!!!!!!!!!!!!!!'
endif
...
if (0 NE n_elements(normalize_density)) then $
				(*st).normalize_density = normalize_density
if keyword_set(norm_absc) then *(*st).norm_absc = norm_absc
if keyword_set(norm_val)  then *(*st).norm_val  = norm_val
