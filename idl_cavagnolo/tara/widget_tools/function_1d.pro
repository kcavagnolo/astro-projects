...
;========================================================================
;;;
;;; Function_1D Widget: $Id: function_1d.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
...
;;; INTERFACE TO CLIENT
;;;
;;; function_1d, top_base, x_data, y_data, ERROR=error, $
...
;		 COLOR=color, LINESTYLE=linestyle, PSYM=psym, PLOTSYM=plotsym,$
;		 DELETE=delete,  ROI_MASK=mask
;;;
...
;;; Errors (standard deviations) may be supplied for a dataset by passing
;;; in ERROR a vector of the same length as x_data & y_data.
...
dataset = { index:0B, name:'', description:'', hidden_flag:0, disable_delete:0,$
	    ...
	    e_data:ptr_new(/ALLOC),  error_available:0, $
	    
	    x_filtered_data:ptr_new(/ALLOC), y_filtered_data:ptr_new(/ALLOC), $
	    e_filtered_data:ptr_new(/ALLOC), $
	    ...
	    plot: {color:'white', psym:3, plotsym:'', line:0, show_errors:0, error_skip:4}, $
...
state = { parent:parent, $ 
	...
	ps_config:ptr_new(/ALLOC) }
	
ptr_free, dataset.x_data, dataset.y_data, dataset.e_data,$
	  dataset.x_filtered_data, dataset.y_filtered_data, $
	  dataset.e_filtered_data, dataset.mask, $
...
for ii = 0, n_elements(state.datasets)-1 do begin
  ...
  state.datasets[ii].e_data   = ptr_new([0])
  state.datasets[ii].x_filtered_data = ptr_new([0])
  state.datasets[ii].y_filtered_data = ptr_new([0])
  state.datasets[ii].e_filtered_data  = ptr_new([0])
  state.datasets[ii].mask            = ptr_new(/ALLOC)
  state.datasets[ii].integral.samples= ptr_new(/ALLOC)
  
  state.datasets[ii].plot.color = color_names[(ii/2) mod nc]
  state.datasets[ii].plot.line  =       lines[ ii    mod nl]
endfor
...
ptr_free, (*st).datasets.x_data, (*st).datasets.y_data, (*st).datasets.e_data,$
          (*st).datasets.x_filtered_data, (*st).datasets.y_filtered_data, $
          (*st).datasets.e_filtered_data, $
...
if (roi_mode EQ 0 ) then begin
  ...
  if (count EQ 0) then begin
    ...
    e_roi_data = ptr_new(/ALLOC)
  endif else begin
    x_roi_data = ptr_new( (*x_data)[index], /NO_COPY)
    y_roi_data = ptr_new( (*y_data)[index], /NO_COPY)
    if dataset.error_available $
      then e_roi_data = ptr_new( (*e_data)[index], /NO_COPY) $
      else e_roi_data = ptr_new(/ALLOC)
  endelse
endelse
...
if (dataset.filter_stale) then begin
  ...
  if (dataset.e_filtered_data NE dataset.e_data) then $
	ptr_free, dataset.e_filtered_data
...
case (mode) of
 ...
 ;-------------------------------------------------------------------------
  ...
  for ii = 0, n_elements(plotsets)-1 do begin
    ...
    if (dataset.num_data_points GT 0) then begin
      ...
      if keyword_set(p.plotsym) then begin
        ...
        if NOT execute(cmd) then message, 'call to plotsym.pro failed'
      endif
      ...
      if (p.line EQ 6) then begin
        ...
        if (p.show_errors AND dataset.error_available) then begin
          oploterror, *dataset.x_filtered_data, *dataset.y_filtered_data, *dataset.e_filtered_data, $
        	              PSYM=p.psym, COLOR=plot_color, ERRSTYLE=0, ERRCOLOR=plot_color, NSKIP=p.error_skip, /NOHAT
        endif else begin
        ...
        if (p.show_errors AND dataset.error_available) then begin
          oploterror, *dataset.x_filtered_data, *dataset.y_filtered_data, *dataset.e_filtered_data, $
        	              LINESTYLE=p.line, PSYM=p.psym, COLOR=plot_color, ERRCOLOR=plot_color, NSKIP=p.error_skip, /NOHAT
        endif else begin
          oplot,      *dataset.x_filtered_data, *dataset.y_filtered_data, $
  ...
  if (lstyle NE 0) then begin
        ...
        legend, plotsets.description, PSYM=psym,  $
                LINE=plotsets.plot.line, $
                /TOP,LEFT=(lstyle EQ 1),CEN=(lstyle EQ 2),RIGHT=(lstyle EQ 3),$
                COLORS=colors, TEXTCOLORS=colors, CHARSIZE=0.75
  endif
  end
...
(*st).fit_menu: $
 ...
 if (selected_dataset.error_available) then begin
   dum = where( *selected_dataset.e_filtered_data LE 0, count )
   if (count EQ 0) then begin
     ...
     weights = 1.0 / (*selected_dataset.e_filtered_data^2) 
     weighted_fit = 1
   endif 
 endif
    ...
    2: $ ; SAVE FUNCTION as FITS
     ...
     if (pathname NE '') then begin
       ...
       if (Error NE 0) then begin
         ...
         sxaddpar, pheader, "CREATOR", "$RCSfile: function_1d.pro,v $, Version $Revision: 1.2 $"
	 ...
	 writefits, pathname, no_data, pheader
         ...
         sxaddpar, fits_header, "CREATOR", "$RCSfile: function_1d.pro,v $, Version $Revision: 1.2 $"
	 ...
	 if (selected_dataset.error_available) then begin
	   bin_table = replicate( {X: 0.0, Y:0.0, YERROR:0.0}, $
	 ...
	 if (selected_dataset.error_available) then $         
           bin_table.YERROR = *selected_dataset.e_filtered_data
	 ...
	 if (selected_dataset.error_available) then begin
	   ...
	   for ii = 0L, selected_dataset.num_data_points-1 do $
	     printf, Unit, (*selected_dataset.x_filtered_data)[ii], $
	     		   ...
	     		   (*selected_dataset.e_filtered_data)[ii]   
	 endif else begin
	   printf, unit, '        X          Y'
	   for ii = 0L, selected_dataset.num_data_points-1 do $
	     printf, Unit, (*selected_dataset.x_filtered_data)[ii], $
...
PRO function_1d, top_base, x_data, y_data, ERROR=error, $
		 TITLE=title,   SUBTITLE=subtitle, $
		 XTITLE=xtitle, YTITLE=ytitle, UNITY_ASPECT=unity_aspect, $
		 XWCS=xwcs, YWCS=ywcs, $
		 NO_SELECT=no_select, $
		 ...
		 HIDE_DATASET=hide_dataset, DISABLE_DELETE=disable_delete, $
		 COLOR=color, LINESTYLE=linestyle, PSYM=psym, PLOTSYM=plotsym,$
		 DELETE=delete,  ROI_MASK=mask, _EXTRA=extra
...
if (create_flag) then top_base = CreateFunction1d( _EXTRA=extra )
...
;; Find the specified dataset or a spot to put a new one.
  if (NOT keyword_set(dataset_name)) then begin
  ...
  if (N_ELEMENTS(x_data) NE 0) then begin
    ...
    *dataset.y_data = y_data 
    ...
    dataset.error_available = (n_elements(error) EQ dataset.num_data_points)
    if dataset.error_available then begin
      *dataset.e_data = error
...
if (new_dataset_flag AND dataset.hidden_flag EQ 0) then begin
  if (NOT keyword_set(no_select)) then (*st).selected_name = dataset.name
  if (dataset_index GT 0)         then (*st).legend_style = 1
endif
