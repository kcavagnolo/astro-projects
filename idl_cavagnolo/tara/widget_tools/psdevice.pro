...
;========================================================================
;;;
;;; FILE NAME:    $Id: psdevice.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
...
;   spawn, '/bin/rm -f idl.ps >& /dev/null'
    ...
    if (Error NE 0) then begin
      ...
      color_manager, PS_GREY=    ((*state.config).color_mode EQ 0), $
		     ...
		     FILENAME=filename, ENCAPSULATED=1, PREVIEW=0, $
    ...
    if (*state.config).inches_t then begin
      ...
      page_long_dim  = 11  * 2.54
      page_short_dim = 8.5 * 2.54
    endelse
    ...
    if (Error EQ 0) then color_manager, XOFFSET=xoffset, YOFFSET=yoffset, $
    					XSIZE=xsize, YSIZE=ysize, $
    					INCHES=(*state.config).inches_t
...
PRO PsDevice, parent, filename, success, CONFIG=config, $
...
if keyword_set(color) then color_mode=2 $
		      else color_mode=1

if (NOT keyword_set(config)) then begin
  config = {color_mode:color_mode, printtoprinter:1, filename:'idl.ps', $
  	    aspect_ratio:1.0, aspect_mode:0, xmargin:0.0, ymargin:0.0, $
  	    xsize:4.5, ysize:9.0, inches_t:1, $
  	    landscape_t:0, bits_per_pixel:8, success:0}
endif
