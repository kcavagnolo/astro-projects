...
;========================================================================
;;;
;;; Dataset_2D Widget: $Id: dataset_2d.pro,v 1.2 2008-10-15 19:35:51 cavagnolo Exp $
;;;
...
state = { parent:parent,$ 
	...
	xcut_widget:0L, ycut_widget:0L, ps_config:ptr_new(/ALLOC),$
...
if keyword_set(colorbar) then begin
  ...
  xyouts, x_label_pos, y_label_pos, labels, CHARSIZE=0.75
  if keyword_set(log) then $
    xyouts, x_bar_pos+xsize+2*char_size[0], y_bar_pos+ysize/2.0, $
            'log scaling', ALIGN=0.5, ORIENT=90, CHARSIZE=0.75
endif
return
end
...
case (mode) of
 ...
 ;-------------------------------------------------------------------------
  ...
  if (lstyle NE 0) then begin
        ...
        legend, descriptions, LINES=lines, PSYM=psyms,  $
  ...
  (*st).derived_menu: $
   ...
   case Event.value of
    ...
    2: $ ;Radial Distribution from Reference Point
     ...
     id = (*st).theta_widget
     dataset_1d, id, theta*!RADEG, GROUP=top_base, BINSIZ=1,$
     ...
     if (pathname NE '') then begin
       ...
       if (Error NE 0) then begin
         ...
         plot_window, (*st).pw_id, XAXIS=xaxis, YAXIS=yaxis
           ...
           sxaddpar, fits_header, "CREATOR", "$RCSfile: dataset_2d.pro,v $, Version $Revision: 1.2 $"
    ...
    3: $ ; SAVE DATASET
     ...
     if (pathname NE '') then begin
       ...
       if (Error NE 0) then begin
         ...
         sxaddpar, pheader, "CREATOR", "$RCSfile: dataset_2d.pro,v $, Version $Revision: 1.2 $"
...
; Edit button: $
  (*st).edit_button: $
   ...
   case (mode) of
    ...
    f1 = '0, BUTTON, Red|Green|Blue|White,'+$
 	 'EXCLUSIVE, SET_VALUE=' + color_index + ', TAG=color, LABEL_TOP=Color'
...
if (create_flag) then top_base = CreateDataset2d( _EXTRA=extra )
