...
PRO Scatter_Surface, x, y, z, _Extra=extra, XTitle=xtitle, $
   ...
   Landscape=landscape, showLines=showLines
...
showLines = Keyword_Set(showLines)
print, "showline ",showLines
...
FOR j=0,npts-1 DO BEGIN
   ...
   IF showLines EQ 1 THEN $
     line[j] = Obj_New('IDLgrPolyLine', [x[j], x[j]], [y[j], y[j]], [z[j], z[j]], $
...
zAxis->SetProperty, Location=[-0.5,  0.5, 9999.0], ZCoord_Conv=zs
   ...
   ; Convert each line from its internal data coordinate space to my
   ; viewplane coordinate space. Get the internal data coordinates from
   ; the line, and then translate and scale these endpoints into my viewplane
   ; coordinate space. This is the location, specified with the POS keyword,
   ; of the orbs.
