...
;
;  $Id: eopdata.pro,v 1.2 2009-06-19 22:21:48 cavagnolo Exp $
;
;-
...
pro eopdata, jdutc, pmx, pmy, ut1_utc, dpsi, deps, reset=reset, $
  ...
  dpsi = dpsi0(jj) + dt * (dpsi0(jj+1)-dpsi0(jj+1))
  deps = deps0(jj) + dt * (deps0(jj+1)-deps0(jj+1))

  ;; Polar motion parameters
  pmx = pmx0(jj) + dt * (pmx0(jj+1)-pmx0(jj+1))
  pmy = pmy0(jj) + dt * (pmy0(jj+1)-pmy0(jj+1))
