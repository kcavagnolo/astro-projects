...
\begin{abstract}
  ...
  of $\approx 6 \times 10^{45} ~\lum$. The cavity energetics indicate
  RBS 797 is similar to other extremely powerful AGN outbursts like
  Cygnus A, Hercules A, and \ms. Based on the presence of bright rims
  enclosing the cavities, we speculate weak shocking ($M \sim 1.5$) of
  the ICM may have accompanied the outburst. A mass accretion rate of
  $\sim 1 ~\msolpy$ is needed to power the outburst, and it appears
  fueling the AGN via pure mass accretion is more difficult than when
  a maximally-spinning black hole serves as the power source. We also
  present evidence of interaction between the host galaxy optical halo
  and the jets and briefly discuss the possibility of AGN-driven star
  formation.
\end{abstract}
