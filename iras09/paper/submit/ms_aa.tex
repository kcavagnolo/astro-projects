...
\newcommand{\mykeywords}{galaxies: quasars: individual: (\inine) --
  galaxies: clusters: individual: (\rxj) -- galaxies: cooling flows --
  galaxies: formation -- galaxies: Seyfert}
\newcommand{\mytitle}{Direct Evidence of Mechanical and Radiative
  Feedback from the Quasar in \inine}
...
\usepackage{color, graphicx, here, common, ifthen, amsmath, amssymb, 
  natbib, mathptmx, url, times, array, txfonts}
\bibpunct{(}{)}{;}{a}{}{,}
...
\title{\mytitle}
\titlerunning{\mystitle}
\authorrunning{Cavagnolo \etal}

\author{
  K. W. Cavagnolo\inst{\ref{inst1}}\and
  M. Donahue\inst{\ref{inst2}}\and
  G. M. Voit\inst{\ref{inst2}}\and
  B. R. McNamara\inst{\ref{inst3},\ref{inst4},\ref{inst5}}\and
  M. Sun\inst{\ref{inst6}}
}

\institute{
  Observatoire de la C\^{o}te d'Azur, UNS, CNRS UMR 6202 Cassiop\'{e}e, Nice, France.\email{cavagnolo@oca.eu}\label{inst1}\and
  Michigan State University, Department of Physics and Astronomy, East Lansing, MI, USA.\label{inst2}\and
  University of Waterloo, Department of Physics and Astronomy, Waterloo, ON, Canada.\label{inst3}\and
  Perimeter Institute for Theoretical Physics, Waterloo, ON, Canada.\label{inst4}\and
  Harvard-Smithsonian Center for Astrophysics, Cambridge, MA, USA.\label{inst5}\and
  University of Virginia, Department of Astronomy, Charlottesville, VA, USA.\label{inst6}\\
}
...
\abstract{
  Using 77 ks of new data from the Chandra X-ray Observatory, we
  present a detailed study of the ultraluminous infrared brightest
  cluster galaxy \inine. Our X-ray analysis reveals the obscured
  active galactic nucleus (AGN) of \inine\ has excavated cavities in
  the host galaxy cluster intracluster medium (ICM) and is presently
  beaming quasar-like radiation into the halo, photoionizing a
  galactic nebula and the ICM. From the ICM cavity properties, we
  estimate the mechanical AGN power output at $\approx 3 \times
  10^{44} ~\lum$, while the nuclear X-ray emission suggests a
  radiative AGN power output in excess of $5 \times 10^{46} ~\lum$,
  \ie\ a radiative to mechanical output ratio of $\approx 200$:1. In
  addition to the direct evidence of mechanical and radiative feedback
  impacting the extended \inine\ halo, we argue that the galaxy dust
  and gas content possibly indicate significant galactic-scale
  radiative feedback effects. From these results, we suggest that
  \inine\ demonstrates the typical characteristics of systems
  experiencing ``quasar-mode'' and ``radio-mode'' feedback, which we
  take as an indication that \inine\ is a unique transition object. We
  go on to suggest that \inine\ may be a local example of how massive,
  high-redshift galaxies transition (or cycle) from a
  radiatively-dominated to a mechanically-dominated form of
  feedback. Using the misalignment between the beamed nuclear
  radiation and the large-scale jets as a constraint, we explore a
  range of possible explanations for the nature of \inine, such as a
  supercritical accretion event and associated evolution of the
  supermassive black hole spin state.
}
...
(\dataset[ADS/Sa.CXO#Obs/00509]{ObsID 509}). Both datasets were
...
~\pcmsq$ \citep{1993MNRAS.263..314L, 2005A&A...444..119G,
  2010ApJ...717..787C}. Assuming the obscuring nuclear material is
...
{\it Facilities:} \facility{CXO ()}, \facility{VLA ()}
