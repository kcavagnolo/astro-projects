...
\begin{abstract}
  Using new data from the \chandra\ X-ray Observatory, we present a
  detailed study of the AGN activity associated with the ultraluminous
  infrared brightest cluster galaxy (BCG) \inine. The X-ray analysis
  reveals the obscured quasar in \inine\ has excavated cavities in the
  host galaxy cluster intracluster medium (ICM) and is beaming intense
  radiation which has photoionized the ICM and a nebula in the
  galaxy's halo. Using the cavity properties, we estimate the
  mechanical AGN power output at $\approx 3 \times 10^{44} ~\lum$,
  while the nuclear X-ray emission suggests a radiative quasar power
  output in excess of $5 \times 10^{46} ~\lum$, giving a radiative to
  mechanical ratio of $\approx$200:1. In addition to the direct
  evidence of mechanical and radiative feedback impacting the extended
  \inine\ halo, we argue that the relative dust-richness and gas
  poorness of \inine\ indicate radiative feedback is significantly
  influencing the host galaxy. From these results, we speculate that
  \inine\ appears to be the first example of a galaxy simultaneously
  undergoing what is often termed ``quasar-mode'' and ``radio-mode''
  ...
  brief period of supercritical accretion and evolution of black hole
  spin. Our interpretation of the observations is that \inine\ is a
  local example of how massive, high-redshift galaxies transition from
  a radiatively-dominated to a mechanically-dominated form of
  feedback.
\end{abstract}
